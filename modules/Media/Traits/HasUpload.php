...
trait HasUpload
{
    ...
    public function uploadFile(Request $request, $fileName = 'file', $fileType = 'image', $folder_id = 0, $allowed_formats = '')
    {
        $this->validateFile($request, $fileType, $fileName, false, $allowed_formats);
    ...
    public function validateFile(Request $request, $group = "default", $fileName = 'file', $isArray = false, $allowed_formats = '')
    {

        $uploadConfigs = config('bc.media.groups');
        $config = isset($uploadConfigs[$group]) ? $uploadConfigs[$group] : $uploadConfigs['default'];

        $ext = !empty($allowed_formats) ? $allowed_formats : implode(',', $config['ext']);
        $rule = ['required', 'file', 'mimes:' . $ext, new FileExtRule(explode(',', $ext))];
